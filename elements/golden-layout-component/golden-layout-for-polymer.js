(function($) {
   ...
   * @returns {VOID}
   */
  lm.LayoutManager = function(config, container, parent) {
  ...
  lm.utils.copy(lm.LayoutManager.prototype, {
    ...
    _bindEvents: function() {
      if (this._isFullPage) {
        ...
        //lm._parent.addEventListener('resize', this._resizeFunction);
      }
    },
    ...
    _adjustToWindowMode: function() {
       ...
       */
      var x = $(lm._parent).offsetHeight; // jshint ignore:line
  ...
  lm.controls.DragProxy = function(x, y, dragListener, layoutManager, contentItem, originalParent) {
    ...
    // CHANGED add lm_parent
    $(lm._parent).append(this.element);

    // CHANGED Changed offset
    //var offset = this._layoutManager.container.offset();
    var offset = {
      left: this._layoutManager.container[0].offsetLeft,
      top: this._layoutManager.container[0].offsetTop
    };
  ...
  lm.controls.TransitionIndicator = function() {
    ...
    // CHANGED add lm._parent
    $(lm._parent).append(this._element);
  ...
  lm.utils.copy(lm.items.AbstractContentItem.prototype, {
    ...
    _$getArea: function(element) {
      ...
      // CHANGED Offset for drop target
      // We get the position of the parent div + the real offset of the target div.
      var width = element.width(),
        height = element.height();

      var offset = {
        left: element[0].offsetLeft + lm._parent.getBoundingClientRect().left,
        top: element[0].offsetTop + lm._parent.getBoundingClientRect().top
      };
  ...
  lm.utils.copy(lm.items.Stack.prototype, {
    ...
    _highlightHeaderDropZone: function(x) {
      ...
      if (tabsLength === 0) {
        ...
        // CHANGED Add parent offset
        headerOffset = {
          left: this.header.element[0].offsetLeft + lm._parent.getBoundingClientRect().left,
          top: this.header.element[0].offsetTop + lm._parent.getBoundingClientRect().top,
        };
      ...
      for (i = 0; i < tabsLength; i++) {
        ...
        while (!tabElement.classList.contains('lm_header')) {
          tabElement = tabElement.offsetParent;
        }

				tabElement = $(tabElement);

        offset = {
          left: tabElement[0].offsetLeft + lm._parent.getBoundingClientRect().left,
          top: tabElement[0].offsetTop + lm._parent.getBoundingClientRect().top,
        };
        // END-CHANGE
      ...
      this.layoutManager.dropTargetIndicator.highlightArea({
        x1: offset.left,//x1: placeHolderLeft,
        x2: offset.left + this.layoutManager.tabDropPlaceholder.width(),//x2: placeHolderLeft + this.layoutManager.tabDropPlaceholder.width(),
        y1: tabTop,
